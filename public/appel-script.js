...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function formatChallengeName(challenge) {
        const challengeMap = {
            'no-presence': 'Aucune présence en ligne',
            'not-enough-clients': 'A une présence mais pas assez de clients',
            'sell-online': 'Vendre en ligne',
            'scale-business': 'Scaler et dominer le marché'
        };
    ...
    function determinePackName(data) {
        ...
        if (priority === 'online-quickly') {
            return 'Pack Présence Rapide';
        } else if (priority === 'generate-leads') {
            return 'Pack Génération de Leads';
        } else if (priority === 'automated-system') {
            return 'Croissance Automatisée';
        } else if (priority === 'optimize-market') {
            return 'Pack Domination Marché';
        }
        
        // Fallback based on challenge (Step 2)
        if (challenge === 'no-presence') {
            return 'Pack Lancement Digital';
        } else if (challenge === 'not-enough-clients') {
            return 'Pack Acquisition Client';
        } else if (challenge === 'sell-online') {
            return 'Pack E-commerce';
        } else if (challenge === 'scale-business') {
    ...
    function updateRecommendations(priority) {
        ...
        if (recommendationText) {
            const recommendations = {
                'online-quickly': 'Établir une présence digitale professionnelle',
                'generate-leads': 'Générer des leads qualifiés immédiatement',
                'automated-system': 'Automatiser votre génération de clients',
                'optimize-market': 'Dominer votre marché avec une stratégie optimisée'
            };
